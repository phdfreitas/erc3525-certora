...
// 1. Transferência entre tokens do mesmo slot deve preservar o saldo total (versão segura)
...
// 2. Transferência para novo token deve incrementar totalSupply
...
// 5. Tokens derivados devem herdar o slot do token original
...
// 7. Transferência para address(0) deve falhar (versão final)
rule transferToZeroAddressFails(uint256 fromTokenId) {
    ...
    // Usando transferFrom(uint256,address,uint256) que retorna uint256
    uint256 newTokenId = transferFrom@withrevert(e, fromTokenId, zeroAddr, value);
    
    assert lastReverted || newTokenId == 0, 
        "Transferência para address(0) deve reverter ou retornar zero";
}
