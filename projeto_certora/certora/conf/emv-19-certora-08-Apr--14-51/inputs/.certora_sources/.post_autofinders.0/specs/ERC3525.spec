...
rule transferToZeroAddressFails(uint256 fromTokenId) {
    ...
    if (lastReverted) {
        // Comportamento esperado (reversão)
        assert true;
    } else {
        // Se não reverteu, deve retornar 0 (comportamento alternativo válido)
        uint256 newTokenId = getLastReturnValue(); 
        assert newTokenId == 0, 
            "Se não reverteu, deve retornar tokenId 0";
    }
}
