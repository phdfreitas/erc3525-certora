...
ghost map uint256 tokenValue;

hook Sstore _values[KEY uint256 tokenId] uint new_value (uint old_value) {
    tokenValue[tokenId] = new_value;
}

invariant tokenValueAlwaysNonNegative(uint256 tokenId) {
    require tokenValue[tokenId] > 0 || tokenValue[tokenId] == 0;
    // Isso evita usar assert com ==
}
