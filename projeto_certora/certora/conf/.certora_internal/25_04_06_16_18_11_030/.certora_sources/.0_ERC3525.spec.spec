...
rule transferValueToNewToken(uint256 fromTokenId, address recipient, uint256 value) {
    ...
    // Substituindo comparações por expressões booleanas equivalentes
    let isSenderOwner = !(owner != e.msg.sender);
    assume isSenderOwner;

    let hasBalance = !(balBefore < value); // em vez de >=
    assume hasBalance;

    let totalCountBefore = totalSupply();
    let newId = transferFrom(e, fromTokenId, recipient, value);

    // Validação principal (preservação do saldo)
