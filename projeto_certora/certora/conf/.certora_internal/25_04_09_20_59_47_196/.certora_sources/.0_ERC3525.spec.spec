...
rule approvedAddressCanTransfer(uint256 tokenId, address approved, address recipient, uint256 value) {
    env e;
    require ownerOf(e, tokenId) == e.msg.sender;

    approve(e, tokenId, approved, value);

    // Cria novo ambiente onde o msg.sender é o endereço aprovado
    env f = env msg.sender: approved

    uint256 ownerBalanceBefore = balanceOf(e, tokenId);
    transferFrom(f, tokenId, recipient, value);

    assert balanceOf(f, tokenId) == ownerBalanceBefore - value;
}
