...
rule transferValueToNewToken(uint256 fromTokenId, address recipient, uint256 value) {
    ...
    uint256 balBefore = balanceOf(fromTokenId);
    ...
    // Pós-condições esperadas se sucesso:
    assert balanceOf(fromTokenId) == balBefore - value, 
        "Saldo do token de origem deve diminuir pelo valor transferido";
    assert ownerOf(newId) == recipient, 
        "Novo token criado deve pertencer ao destinatário";
    assert balanceOf(newId) == value, 
        "Novo token deve conter exatamente o valor transferido";
    assert slotOf(newId) == slotOf(fromTokenId), 
        "O slot do novo token deve ser igual ao slot do token de origem";
    assert totalSupply() == totalCountBefore + 1, 
        "Total de tokens emitidos deve aumentar em 1 após criar novo token";
}
